...
def train(model, optimizer, weight_calculator, loss_fn, train_dataloader, test_dataloader, epochs, alg, device, adaptive):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            model.zero_grad()
            if weight_calculator.noise_type in ('gradient_add', 'gradient_flip'):
        ...
        (tr_loss, tr_acc) = output(train_dataloader, model, alg, device, train=False)
...
def make_noisy_data_label(train_data, noise_type, noise_frac, shift=1):
    '''
