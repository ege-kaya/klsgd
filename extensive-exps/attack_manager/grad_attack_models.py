...
class Additive(ByzAttack):
    ...
    def attack(self, g):
        if self.noise_dist == 'gaussian':
            noise = torch.randn(g.shape, generator=self.random_state).to(g.device) * self.attack_std + g * self.mean_shift
        elif self.noise_dist == 'uniform':
            noise_ub = self.noise_range[1]
            noise_lb = self.noise_range[0]
            dist = noise_ub - noise_lb
            noise = torch.rand(g.shape, generator=self.random_state) * dist + noise_lb
        else:
            raise NotImplementedError
