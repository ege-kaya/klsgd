...
class KLSGD(torch.optim.Optimizer):
    def __init__(self, params, robust=True, lr=1e-3, reg=0.5, batch_size=16):
        if lr < 0.0:
        ...
        defaults = dict(lr=lr, reg=reg, batch_size=batch_size)
        self.lr = lr
        self.reg = reg
        self.robust=robust
        ...
        self.weights = []
        self.dir = -1 if self.robust is True else 1

    def calc_norm(self):
        with torch.no_grad():
            for grads in self.batch_gradients:
                all_gradients = torch.cat([grad.flatten() for grad in grads])
                l2_norm = torch.norm(all_gradients, p=2)
                weight = torch.exp(self.dir * self.lr * l2_norm ** 2 / self.reg)
                self.weights.append(weight)
    
    def step(self, closure=None):
        # calculate the l2 norm 
        self.calc_norm()
        K = len(self.batch_gradients) # batch size 
        # calculate the normalizer 
        # Z = torch.sum(torch.tensor(self.weights))
        for group in self.param_groups:
            ...
            for p in group["params"]:
                if p.grad is None:
                    continue
                idx += 1
                update = torch.zeros_like(self.batch_gradients[0][idx])
                for i in range(K):
                    update.add_(self.weights[i] * self.batch_gradients[i][idx])
                p.data.add_(self.dir * self.lr * update)
        # reset the batch gradients and weights 
        self.batch_gradients = []
        self.weights = []    
...
class KLSGD_ERM(ERM):
    """
    Empirical Risk Minimization with KLSGD
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(KLSGD_ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for j in range(loss.shape[0]):
            ...
            self.optimizer.batch_gradients.append([param.grad.data for param in self.network.parameters() if param.grad is not None])
